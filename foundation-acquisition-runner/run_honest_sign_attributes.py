...
import hashlib, html, json, pathlib, re, time, urllib.request, urllib.parse, zipfile
...
UA='Mozilla/5.0 (X11; Linux x86_64) ImportCost-Regulatory-Foundation/1.0'
def fetch(url, attempts=3):
    ...
    for i in range(1,attempts+1):
        try:
            ...
            hist.append({'attempt':i,'ok':False,'error':type(e).__name__+': '+str(e)[:500]}); time.sleep(i*2)
    return None,None,None,{},hist
page,status,final,hdr,idx_attempts=fetch(INDEX)
...
for n,url in enumerate(links,1):
    ...
    results.append({'ordinal':n,'url':url,'finalUrl':final,'httpStatus':status,'contentType':hdr.get('content-type'),
...
summary={'authority':'CRPT / GIS MT','sourceRole':'OFFICIAL_OPERATIONAL_SOURCE','indexUrl':INDEX,
 ...
 'expectedByFoundation':44,'allFetched':len(links)==44 and all(r['acquired'] for r in results),'records':results}
